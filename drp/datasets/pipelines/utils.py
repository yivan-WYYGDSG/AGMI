...
def get_edges_attr(path):
    edges = np.load(path, allow_pickle=True).item()
    edges_index = edges['edge_index']
    edges_attr = edges['edge_atrr']
    return edges_index, edges_attr


def get_edges(path):
    edges = np.load(path, allow_pickle=True)
    return edges
...
def get_weight(GSEA_PATH, PPI_PATH, PEARSON_PATH):
    PPI = get_edges_attr(PPI_PATH)
    GSEA = get_edges_attr(GSEA_PATH)
    pearson_data = get_edges(PEARSON_PATH)
    # PPI_data = torch.LongTensor(PPI[0])
    edge_index = torch.cat((torch.LongTensor(PPI[0]), torch.LongTensor(GSEA[0]), torch.LongTensor(pearson_data)), dim=1)
    edges_nums = edge_index.shape[1]
    edge_attr = [MaxMinScalr(torch.FloatTensor(PPI[1])) + 1,
                 MaxMinScalr(torch.FloatTensor(GSEA[1])) + 1,
                 torch.ones(pearson_data.shape[1])]
    weight = [torch.zeros(edges_nums) for _ in range(3)]
    cur_ind = 0
    for ind, w in enumerate(weight):
        w[cur_ind:cur_ind + len(edge_attr[ind])] = edge_attr[ind]
        cur_ind += len(edge_attr[ind])
